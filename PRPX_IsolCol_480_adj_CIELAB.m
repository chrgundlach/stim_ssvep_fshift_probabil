function [AdjConditions]=PRPX_IsolCol_480_adj_CIELAB(ColorsIN_RGB,TrialsPerCon,MaxStd,Shift,varargin)
% function [AdjConditions]=PRPX_IsolCol_480_adj_CIELAB(Colors,TrialsPerCon,MaxStd,Shift,varargin)
% script to set colors isoluminant with an additional transformation of color into CIE L*a*b space
% -ColorsIN_RGB: rows with R/G/B values (max 1). Colors will be adjusted to first row.
...
%               - Datapixx('RegWrRd');
% example call:
%   [AdjConditions]=PRPX_IsolCol_480_adj_CIELAB([0.03 0.03 0.03; 1 0 0; 0 0 1],5,20,[0 -300]);
%
% for troubleshooting:
...
if nargin <4
    help PRPX_IsolCol_480_adj_CIELAB
    return
end
...
% set up window
[p.window, p.windowRect] = PsychImaging('OpenWindow', p.screen_num, ColorsIN_RGB(1,:));
...
AdjConditions=lab2rgb([Colors(1,:);ScaleLuminance(Col2Pre(1:end,:),LumAdj)]);
ListenChar(0)
...
LumAdj=floor(rand*1000)/10; % set starting luminance to random value
FrameCounter=0;
StepSize=0.05;
ActivityCount=0;
...
while (~key.keycode(Keys(3)) && ~key.keycode_prev(Keys(3))) || key.keycode_prev(Keys(3))
    ...
    if any(key.keycode(Keys(1:2))) % has increase or decrease button been pressed?
        ...
        LumAdj=min([LumAdj+key.keycode(Keys(2))*StepSize 100]); % adjust luminance
        %disp(LumAdj)
    ...
    NewCol_CIELAB=ScaleLuminance(ColAdj,LumAdj);
    NewCol_RGB = lab2rgb(NewCol_CIELAB);
    
    % draw ellipses
    for i_quad = 1:4
        FrameCounter=mod(FrameCounter+1,FreqDiv); % according to on/off frequency
        if FrameCounter<(FreqDiv/2)
            OvalCol = [NewCol_RGB(1),NewCol_RGB(2),NewCol_RGB(3)]; % color of on frame
        else
